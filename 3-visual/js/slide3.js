$(document).ready(function(){
  ...
  //슬라이드배치
  //각각 할일이 있고, each-매개변수는 각각의 indx번호를 사용할수 있음(요소검사에서 배치확인)
  slides.each(function(idx){
    $(this).css({left:`${idx*100}%`}); //0%, 100%, 200%.... 
  });
  ...
  function moveSlide(i){
    if(currentIdx === i) return; // 현재 0번인데 새로 클릭한게 0번이면 같으므로 아무것도 실행하지 않고 나감(return문 아래 문장을 실행하지 않고 나감)
