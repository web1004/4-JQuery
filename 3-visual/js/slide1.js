$(document).ready(function(){
  ...
  function horizontalSlider(){
    hautoplay = setInterval(function(){
      $(".slide-horizontal-items").stop().animate({left: "-940px"},function(){
  ...
  function  verticalSlider(){
    vautoplay = setInterval(function(){
      $(".slide-vertical-items").stop().animate({top: "-430px"},function(){
        $('.slide-vertical-items').css({top: 0});  //최종위치
        $('.slide-vitem:first-child').appendTo('.slide-vertical-items'); // 첫번째를 맨뒤로
      }); 
    },4000);
  };
  verticalSlider();
